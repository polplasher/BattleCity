class playerPrefab extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, key = 'player') {
        ...
        this.facing = 'down';
        this.lastShot = 0;
        this.speed = gamePrefs.TANK_SPEED;
        this.scene = scene;
    ...
    createAnimations() {
        ...
        animConfigs.forEach(config => {
            this.scene.anims.create({
                ...
                frameRate: gamePrefs.TANK_FRAME_RATE,
                repeat: -1
            });
        });
    }
    ...
    canShoot() {
        ...
        if (activeCount >= gamePrefs.MAX_PLAYER_BULLETS) return false;

        const now = this.scene.time.now;
        if (now - this.lastShot < gamePrefs.FIRE_COOLDOWN) return false;

        return true;
    }

    getBulletSpawnData() {
        const speed = gamePrefs.BULLET_SPEED;
    ...
    createBulletFromPool(vx, vy) {
        ...
        if (!bullet) {
            bullet = new bulletPrefab(this.scene, this.x, this.y, 'bullet');
